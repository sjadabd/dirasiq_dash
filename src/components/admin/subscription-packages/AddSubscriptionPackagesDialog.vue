  ...
  methods: {
    ...
    async saveNote() {
      ...
      try {
        ...
        const response = await adminApi.createSubscriptionPackage(bankData);

        this.$emit(
          "showAlert",
          "success",
          response.data.message || "تم إضافة الباقة بنجاح"
        );

        this.$emit("dataAdded", response.data.message);
        this.closeDialog();
      } catch (error) {
        let errorMessage = "حدث خطأ أثناء إضافة الباقة";
