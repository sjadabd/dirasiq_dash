...
export default {
  ...
  methods: {
    ...
    async submitAddAttendees() {
      ...
      if (toAdd.length === 0) { this.showAlert("info", "لا توجد إضافات جديدة"); this.attendeesDialog.open = false 
  ...
  <div>
        ...
        >
          <VCardText>
            ...
            <div class="text-h5 font-weight-bold text-success">
              {{ numberWithComma(totalAttendees) }}
            </div>
            <div class="text-caption text-medium-emphasis">
              مجموع الحضور
            </div>
    ...
    >
      ...
      <div class="text-h6 mb-2">
        لا توجد جلسات بهذا الفلتر
      </div>
      <div class="text-body-2 text-medium-emphasis mb-4">
        ابدأ بإضافة جلستك الأولى لتظهر في الجدول الأسبوعي.
      </div>
      <VBtn
        color="primary"
        prepend-icon="ri-add-line"
        ...
        <VCardText class="pa-3">
          <div class="schedule-grid">
            ...
            >
              ...
              >
                <div
                  v-if="row.byDay[d.value].length"
                  class="schedule-cell-stack"
                >
                  <VMenu
                    v-for="s in row.byDay[d.value]"
                    :key="s.__key"
                  ...
                  >
                    ...
                    <VList density="compact">
                      <VListItem
                        ...
                        @click="openViewStudentsDialog(s)"
                      >
                        عرض الطلاب
                      </VListItem>
                      <VListItem
                        prepend-icon="ri-user-add-line"
                        @click="openAddAttendeesDialog(s)"
                      >
                        إضافة طلاب
                      </VListItem>
                      <VListItem
                        prepend-icon="ri-calendar-check-line"
                        @click="navigateToAttendance(s)"
                      >
                        سجل الحضور
                      </VListItem>
                      <VListItem
      ...
      <div class="d-block d-md-none">
        ...
        >
          <VCardTitle class="d-flex align-center py-3 px-4">
            <VIcon
              :icon="day.isToday ? 'ri-calendar-event-line' : 'ri-calendar-line'"
              :color="day.isToday ? 'warning' : 'primary'"
              class="me-2"
            />
            <span>{{ day.text }}</span>
            <VChip
      ...
      <VCardText>
        <div class="d-flex align-center mb-3">
          ...
          <VChip
            ...
            size="small"
          >
            {{ numberWithComma(items.length) }} جلسة
          </VChip>
        </div>
        <SmartTable
          :headers="headers"
          ...
          @show-item="navigateToAttendance"
          @show-attendees-item="openViewStudentsDialog"
          @add-attendees-item="openAddAttendeesDialog"
          @end-session-item="openEndSessionDialog"
        />
      </VCardText>
    </VCard>
    ...
    >
      <VCard>
        <VCardTitle class="d-flex align-center">
          <VIcon
            :icon="sessionDialog.mode === 'create' ? 'ri-add-circle-line' : 'ri-pencil-line'"
            color="primary"
            class="me-2"
          />
        ...
        <VCardText>
          <VAlert
            v-if="sessionDialog.conflictMessage"
            type="error"
            variant="tonal"
            density="compact"
            class="mb-3"
      ...
      <VCard>
        <VCardTitle class="d-flex align-center">
          ...
          <VBtn
            icon="ri-close-line"
            variant="text"
            @click="attendeesDialog.open = false"
          />
        </VCardTitle>
        <VDivider />
        <VCardText>
          ...
          <VSelect
            ...
            :items="attendeesDialog.candidates"
            item-title="name"
            item-value="id"
            label="اختر الطلاب"
            variant="outlined"
            ...
            :disabled="attendeesDialog.loading"
            multiple
            chips
            clearable
            prepend-inner-icon="ri-search-line"
          ...
          <div class="d-flex align-center justify-space-between mb-2">
            ...
            <VChip
              ...
              variant="tonal"
            >
              {{ attendeesDialog.currentDetailed.length }}
            </VChip>
          </div>
          <div
            v-if="attendeesDialog.loading"
            class="text-medium-emphasis"
          >
            جاري التحميل...
          </div>
          <div
            v-else-if="!attendeesDialog.currentDetailed.length"
            class="text-medium-emphasis"
          >
            لا يوجد طلاب بعد.
          </div>
          ...
          >
            <VChip
              v-for="st in attendeesDialog.currentDetailed"
              :key="st.student_id || st"
              size="small"
              variant="outlined"
              closable
    ...
    <VDialog
      v-model="studentsViewDialog.open"
      max-width="720"
    >
      <VCard>
        <VCardTitle class="d-flex align-center">
          ...
          <VBtn
            icon="ri-close-line"
            variant="text"
            @click="studentsViewDialog.open = false"
          />
        </VCardTitle>
        <VDivider />
        <VCardText>
          <div
            v-if="studentsViewDialog.loading"
            class="text-medium-emphasis"
          >
            جاري التحميل...
          </div>
          <VAlert
            v-else-if="!studentsViewDialog.students.length"
            type="info"
            variant="tonal"
            density="compact"
          >
          ...
          <VRow v-else>
            <VCol
              v-for="st in studentsViewDialog.students"
              :key="st.student_id"
              cols="12"
              sm="6"
            >
    ...
    <ConfirmDangerDialog
      ...
      :confirm-button-text="endDialog.confirmButtonText"
