...
<template>
  <div>
    ...
    <div class="att-hero rounded-xl pa-5 pa-md-6 mb-6 d-flex flex-wrap align-center ga-3">
      ...
      <div class="flex-grow-1">
        <div class="d-flex align-center ga-2 flex-wrap">
          ...
          <VChip v-if="isToday" color="warning" size="small" variant="flat">اليوم</VChip>
        </div>
        ...
        <VCard rounded="lg" elevation="0" class="border h-100">
          <VCardText>
            <div class="text-caption text-medium-emphasis mb-2">
              <VIcon icon="ri-calendar-line" size="14" class="me-1" /> اختر التاريخ
            </div>
            <VTextField
            ...
            />
            <div class="text-caption text-medium-emphasis mt-2">{{ formatDate(filters.date) }}</div>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="6" md="2">
        <VCard rounded="lg" elevation="0" class="kpi-card kpi-present h-100">
          <VCardText class="text-center">
            <VIcon icon="ri-check-double-line" color="success" size="28" />
            <div class="text-h5 font-weight-bold text-success mt-2">{{ stats.present }}</div>
            <div class="text-caption text-medium-emphasis">حاضر</div>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="6" md="2">
        <VCard rounded="lg" elevation="0" class="kpi-card kpi-absent h-100">
          <VCardText class="text-center">
            <VIcon icon="ri-close-circle-line" color="error" size="28" />
            <div class="text-h5 font-weight-bold text-error mt-2">{{ stats.absent }}</div>
            <div class="text-caption text-medium-emphasis">غائب</div>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="6" md="2">
        <VCard rounded="lg" elevation="0" class="kpi-card kpi-leave h-100">
          <VCardText class="text-center">
            <VIcon icon="ri-time-line" color="warning" size="28" />
            <div class="text-h5 font-weight-bold text-warning mt-2">{{ stats.leave }}</div>
            <div class="text-caption text-medium-emphasis">إجازة</div>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="6" md="2">
        <VCard rounded="lg" elevation="0" class="kpi-card h-100">
          <VCardText class="text-center">
            <VIcon icon="ri-question-line" color="grey" size="28" />
            <div class="text-h5 font-weight-bold mt-2">{{ stats.unset }}</div>
            <div class="text-caption text-medium-emphasis">لم يُحدّد</div>
          </VCardText>
        </VCard>
      </VCol>
    </VRow>
      ...
      <VCardText>
        <div class="d-flex flex-wrap ga-2 align-center">
          <span class="text-subtitle-2 font-weight-bold me-2">
            <VIcon icon="ri-magic-line" color="primary" class="me-1" /> إجراءات سريعة:
          </span>
          <VBtn color="success" variant="tonal" size="small" prepend-icon="ri-check-double-line" :disabled="!students.length" @click="bulkSet('present')">
            الكل حاضر
          </VBtn>
          <VBtn color="error" variant="tonal" size="small" prepend-icon="ri-close-circle-line" :disabled="!students.length" @click="bulkSet('absent')">
            الكل غائب
          </VBtn>
          <VBtn variant="text" size="small" prepend-icon="ri-eraser-line" :disabled="!students.length" @click="clearAll">
            مسح الكل
          </VBtn>
          <VSpacer />
          <VTextField
            ...
            clearable style="max-width: 280px;"
          />
        </div>
        <div v-if="students.length" class="mt-3 d-flex align-center ga-2">
          <span class="text-caption text-medium-emphasis">نسبة الحضور:</span>
          <VProgressLinear :model-value="presentRate" color="success" height="8" rounded class="flex-grow-1" />
    ...
    <!-- Loading skeleton -->
    <VCard v-if="loading && !students.length" rounded="lg" elevation="0" class="border pa-4">
      <VSkeletonLoader v-for="n in 5" :key="n" type="list-item-avatar-three-line" />
    </VCard>

    <!-- Empty state -->
    <VCard v-else-if="!students.length" rounded="lg" elevation="0" class="border text-center pa-8">
      <VIcon icon="ri-user-search-line" size="64" color="grey" class="mb-3" />
      <div class="text-h6 mb-2">لا يوجد طلاب في هذه الجلسة</div>
      <div class="text-body-2 text-medium-emphasis mb-4">
        أضف الطلاب إلى الجلسة من الجدول الأسبوعي قبل تسجيل الحضور.
      </div>
      <VBtn color="primary" prepend-icon="ri-arrow-go-back-line" @click="$router.push('/teacher/sessions/manage-sessions')">
        العودة للجدول
      </VBtn>
    </VCard>

    <!-- Students list -->
    <VCard v-else rounded="lg" elevation="0" class="border mb-20">
      <VCardText class="pa-2">
        <VAlert v-if="!filteredStudents.length" type="info" variant="tonal" density="compact" class="ma-2">
          لا يوجد طلاب يطابقون "{{ searchTerm }}".
        </VAlert>
        <div v-else class="d-flex flex-column ga-2">
          <div
            v-for="(st, idx) in filteredStudents" :key="st.student_id"
          ...
          >
            <div class="d-flex align-center ga-3 flex-wrap">
              ...
              <div class="flex-grow-1 overflow-hidden">
                <div class="text-subtitle-1 font-weight-bold text-truncate">{{ st.student_name }}</div>
              ...
              <div class="status-seg d-flex">
                <VBtn
                  ...
                  size="small" :prepend-icon="s.icon"
    ...
    <!-- Sticky save bar -->
    <div v-if="students.length" class="save-bar">
      <VCard rounded="lg" elevation="8" class="d-flex align-center pa-3 ga-2">
        ...
        <VSpacer />
        <VBtn variant="text" :disabled="loading || saving" @click="loadAttendance">
          <VIcon icon="ri-refresh-line" class="me-1" />
          استرجاع
        </VBtn>
        <VBtn color="primary" :loading="saving" :disabled="!isDirty" prepend-icon="ri-save-3-line" @click="saveAttendance">
          حفظ الحضور
        </VBtn>
      </VCard>
    </div>
