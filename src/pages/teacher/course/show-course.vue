  ...
  <div>
    ...
    <div class="course-hero rounded-xl pa-5 pa-md-6 mb-6 d-flex flex-wrap align-center ga-3">
      <div class="hero-icon-wrap rounded-circle d-flex align-center justify-center">
        <VIcon icon="ri-book-3-line" size="28" color="white" />
      </div>
      <div class="flex-grow-1">
        <div class="text-h6 text-md-h5 font-weight-bold text-white">كورساتي</div>
      ...
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="kpi-card">
          <VCardText>
            ...
            <div class="text-caption text-medium-emphasis">عدد الكورسات</div>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="kpi-card">
          <VCardText>
            ...
            <div class="text-h5 font-weight-bold text-success">{{ numberWithComma(kpis.activeOnPage) }}</div>
            <div class="text-caption text-medium-emphasis">في هذه الصفحة</div>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="kpi-card">
          <VCardText>
            <div class="d-flex align-center justify-space-between mb-2">
              ...
              <VChip size="small" color="info" variant="flat">مقاعد</VChip>
            </div>
            <div class="text-h5 font-weight-bold">{{ numberWithComma(kpis.totalSeats) }}</div>
            <div class="text-caption text-medium-emphasis">مجموع المقاعد (الصفحة)</div>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="kpi-card">
          <VCardText>
            <div class="d-flex align-center justify-space-between mb-2">
              <VIcon icon="ri-coins-line" color="warning" size="24" />
              <VChip size="small" color="warning" variant="flat">عربون</VChip>
            </div>
            <div class="text-h5 font-weight-bold text-warning">{{ numberWithComma(kpis.withReservation) }}</div>
            <div class="text-caption text-medium-emphasis">كورسات بعربون</div>
          </VCardText>
        </VCard>
      </VCol>
    </VRow>
      ...
      <VCardText>
        <div class="d-flex align-center mb-3 ga-3 flex-wrap">
          ...
          <VBtnToggle v-model="view" mandatory color="primary" variant="outlined" divided class="view-toggle">
            <VBtn value="cards" @click="setView('cards')">
              <VIcon icon="ri-grid-line" start />
              بطاقات
            </VBtn>
        ...
        <VRow dense>
          ...
          <VCol cols="6" md="2">
            <VSelect
              ...
              :items="grades" item-title="title" item-value="value"
              label="الصف" density="comfortable" variant="outlined" clearable
              @update:model-value="onFilterChange"
            />
          </VCol>
          <VCol cols="6" md="2">
            <VSelect
              ...
              :items="subjects" item-title="title" item-value="value"
              label="المادة" density="comfortable" variant="outlined" clearable
              @update:model-value="onFilterChange"
            />
          </VCol>
          <VCol cols="6" md="2">
            <VSelect
              ...
              label="الحالة" density="comfortable" variant="outlined"
              @update:model-value="onFilterChange"
            />
          </VCol>
        </VRow>
    ...
    <template v-if="view === 'cards'">
      ...
      <VCard v-else-if="!items.length" rounded="lg" elevation="0" class="border text-center pa-8">
        <VIcon icon="ri-book-open-line" size="64" color="grey" class="mb-3" />
        <div class="text-h6 mb-2">لا يوجد كورسات بهذا الفلتر</div>
        <div class="text-body-2 text-medium-emphasis mb-4">
          {{ isAnyFilter ? "جرّب تغيير الفلاتر أو" : "ابدأ بإضافة" }} كورسك الأول
        </div>
        <VBtn color="primary" prepend-icon="ri-add-line" @click="openAdd">إضافة كورس</VBtn>
      </VCard>

      <!-- Cards -->
      <VRow v-else>
        <VCol v-for="item in items" :key="item.id" cols="12" sm="6" md="4">
          <VCard rounded="lg" elevation="0" class="course-card h-100 d-flex flex-column">
            <!-- Image -->
            <div class="course-image-wrap">
              <VImg v-if="courseImage(item)" :src="courseImage(item)" cover height="160" class="rounded-t-lg">
                <template #placeholder>
                  <div class="d-flex align-center justify-center fill-height bg-grey-lighten-3">
                    <VIcon icon="ri-image-line" size="36" color="grey" />
                  </div>
                </template>
                <template #error>
                  <div class="d-flex align-center justify-center fill-height bg-grey-lighten-3">
              ...
              <div v-else class="d-flex align-center justify-center bg-image-fallback rounded-t-lg" style="height: 160px;">
                <VIcon icon="ri-book-3-line" size="48" color="white" />
              </div>
              <!-- Status chip overlay -->
              <VChip
                :color="statusOf(item).color" size="small" variant="flat"
                :prepend-icon="statusOf(item).icon"
                class="status-chip"
              >{{ statusOf(item).label }}</VChip>
              <!-- Reservation badge -->
              <VChip
                v-if="item.has_reservation" color="warning" size="small" variant="flat"
                prepend-icon="ri-coins-line" class="reservation-chip"
              >{{ formatIQDShort(item.reservation_amount) }}</VChip>
            </div>

            <VCardText class="flex-grow-1">
              <div class="text-h6 font-weight-bold mb-1 text-truncate" :title="item.course_name">{{ item.course_name }}</div>
              <div class="d-flex ga-1 mb-3 flex-wrap">
                <VChip size="x-small" color="info"    variant="tonal" prepend-icon="ri-graduation-cap-line">{{ item.grade_name || "—" }}</VChip>
                <VChip size="x-small" color="primary" variant="tonal" prepend-icon="ri-book-open-line">{{ item.subject_name || "—" }}</VChip>
              </div>
              ...
              <div class="d-flex align-center justify-space-between mb-1">
                <div class="d-flex align-center">
                  <VIcon icon="ri-money-dollar-circle-line" size="16" color="success" class="me-1" />
                  <span class="font-weight-bold">{{ formatIQD(item.price) }} د.ع</span>
                </div>
                <div class="d-flex align-center text-caption">
                  <VIcon icon="ri-group-line" size="14" class="me-1" />
            ...
            <VCardActions class="pa-2">
              <VBtn
                v-if="!isDeleted(item)" color="primary" variant="tonal" size="small"
                prepend-icon="ri-group-line" @click="handleShowCourseStudents(item)"
              >الطلاب</VBtn>
              <VSpacer />
              <VBtn
                v-if="!isDeleted(item)" icon="ri-pencil-line" size="small" variant="text" color="info"
                @click="editItem(item)"
              />
              <VBtn
                v-if="!isDeleted(item)" icon="ri-delete-bin-line" size="small" variant="text" color="error"
                @click="deleteItem(item)"
              />
              <VBtn
                v-else color="success" variant="tonal" size="small"
                prepend-icon="ri-refresh-line" @click="enableItem(item)"
              >استرجاع</VBtn>
            </VCardActions>
          </VCard>
        </VCol>
      ...
      <!-- Pagination -->
      <div v-if="totalItems > tableOptions.limit" class="d-flex justify-center mt-6">
        <VPagination
    ...
    <VCard v-else rounded="lg" elevation="0" class="border">
      <VCardText>
        <div class="d-flex align-center mb-3">
          ...
          <VSpacer />
          <VChip color="primary" variant="flat" size="small">{{ numberWithComma(totalItems) }} سجل</VChip>
        </div>
        <SmartTable
          :headers="headers" :items="items"
          :actions="tableActions" :loading="loading"
          :totalItems="totalItems" :tableOptions="tableOptions"
          @updateTableOptions="onUpdateTableOptions"
          @deleteItem="deleteItem" @editItem="editItem"
          @enableItem="enableItem" @showItem="handleShowCourseStudents"
        />
      </VCardText>
    </VCard>
    ...
    />
    <EditCourse
      v-if="editGrades.open" v-model="editGrades.open" :courseData="editGrades.data"
