<template>
  <div>
        ...
        <VRow style="padding-block: 10px;">
          <VCol cols="12" md="4">
            <VSelect v-model="table.tableSettings.options.is_deleted" :items="courseIsDisabled" item-title="text"
              item-value="value" label="حالة الكورس" variant="outlined" @update:model-value="getDataAxios" />
          </VCol>
        </VRow>
      </VCardItem>
    </VCard>
    ...
    <VCard class="my-4 data-table-card" elevation="3" rounded="lg">
      <VCardTitle class="py-4 px-6">
        <VRow class="align-center">
          ...
          <VCol>
            <h3 class="text-h5 font-weight-bold text-center">الكورسات</h3>
          </VCol>
          <VCol cols="auto">
    ...
    <!-- Add course Dialog -->
    <AddCourse v-if="Actions.open" v-model="Actions.open" @close="Actions.open = false" @dataAdded="handleDataAdded"
      @showAlert="showAlert" />
    <!-- Add grades Dialog -->

    <!-- Add Course Dialog -->
    <EditCourse v-if="editGrades.open" v-model="editGrades.open" :courseData="editGrades.data"
  ...
  methods: {
    ...
    async getDataAxios() {
      ...
      try {
        ...
        const response = await TeacherApi.getCourse(this.table.tableSettings);
