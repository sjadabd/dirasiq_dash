  ...
  <div>
    ...
    <div class="eval-hero rounded-xl pa-5 pa-md-6 mb-6 d-flex flex-wrap align-center ga-3">
      <div class="hero-icon-wrap rounded-circle d-flex align-center justify-center">
        <VIcon icon="ri-star-smile-line" size="28" color="white" />
      </div>
      <div class="flex-grow-1">
        <div class="text-h6 text-md-h5 font-weight-bold text-white">تقييمات الطلاب</div>
      ...
      <VBtn color="warning" prepend-icon="ri-add-line" size="large" rounded="pill" @click="goToBulk">
        إضافة تقييمات
      </VBtn>
    </div>
      ...
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="kpi-card">
          <VCardText>
            ...
            <div class="text-h5 font-weight-bold">{{ Number(totalItems).toLocaleString() }}</div>
            <div class="text-caption text-medium-emphasis">تقييم مسجّل</div>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="kpi-card">
          <VCardText>
            <div class="d-flex align-center justify-space-between mb-2">
              ...
              <VChip size="small" color="info" variant="flat">طلاب</VChip>
            </div>
            <div class="text-h5 font-weight-bold">{{ uniqueStudents }}</div>
            <div class="text-caption text-medium-emphasis">طالب مختلف</div>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="kpi-card">
          <VCardText>
            <div class="d-flex align-center justify-space-between mb-2">
              <VIcon icon="ri-line-chart-line" color="success" size="24" />
              <VChip size="small" :color="scoreColor(avgScore)" variant="flat">{{ scoreDisplay(avgScore) }}</VChip>
            </div>
            <div class="text-h5 font-weight-bold" :class="`text-${scoreColor(avgScore)}`">{{ avgScore }}</div>
            <div class="text-caption text-medium-emphasis">المعدّل العام</div>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="kpi-card">
          <VCardText>
            <div class="d-flex align-center justify-space-between mb-2">
              <VIcon icon="ri-calendar-line" color="warning" size="24" />
              <VChip size="small" color="warning" variant="flat">آخر</VChip>
            </div>
            <div class="text-h6 font-weight-bold">{{ formatDate(latestDate) }}</div>
            <div class="text-caption text-medium-emphasis">آخر تاريخ تقييم</div>
          </VCardText>
        </VCard>
      </VCol>
    </VRow>
        ...
        <VRow dense>
          <VCol cols="12" md="5">
            <VTextField
              ...
              @click:clear="onSearch" @input="onSearch"
            />
          </VCol>
          <VCol cols="12" md="3">
    ...
    <VRow v-if="loading && !items.length">
      <VCol v-for="n in 6" :key="n" cols="12" md="6">
    ...
    <VCard v-else-if="!items.length" rounded="lg" elevation="0" class="border text-center pa-8">
      <VIcon icon="ri-star-line" size="64" color="grey" class="mb-3" />
      <div class="text-h6 mb-2">لا توجد تقييمات بهذا الفلتر</div>
      <div class="text-body-2 text-medium-emphasis mb-4">
        ابدأ بإضافة تقييمات اليوم لمتابعة تقدّم طلابك.
      </div>
      <VBtn color="primary" prepend-icon="ri-add-line" @click="goToBulk">إضافة تقييمات</VBtn>
    </VCard>

    <!-- CARDS view -->
    <template v-else-if="view === 'cards'">
      <VAlert v-if="!filteredItems.length" type="info" variant="tonal" density="compact" class="mb-4">
        لا يوجد طلاب يطابقون "{{ searchTerm }}".
      </VAlert>
      <VRow>
        <VCol v-for="it in filteredItems" :key="it.id" cols="12" md="6">
          <VCard rounded="lg" elevation="0" class="eval-card h-100">
            <VCardText>
              <!-- Header: student + course + date + avg -->
              <div class="d-flex align-center ga-3 mb-3">
                <VAvatar :color="scoreColor(it.avg)" size="44">
                  <span class="text-subtitle-1 font-weight-bold">{{ studentInitials(it.student_name) }}</span>
                </VAvatar>
                <div class="flex-grow-1 overflow-hidden">
                  <div class="text-subtitle-1 font-weight-bold text-truncate">{{ it.student_name }}</div>
                  <div class="text-caption text-medium-emphasis text-truncate">
                    <VIcon icon="ri-book-3-line" size="12" class="me-1" />
                    {{ it.course_name }}
                ...
                <div class="text-end">
                  <VChip :color="scoreColor(it.avg)" variant="flat" size="default">
                    <VIcon icon="ri-trophy-line" size="14" start />
                    {{ it.avgDisplay }}
                  </VChip>
                  <div class="text-caption text-medium-emphasis mt-1">المعدّل</div>
                </div>
              </div>

              <!-- Axis chips: per-axis Arabic rating label with score-derived color -->
              <div class="d-flex flex-wrap ga-2">
                <div v-for="a in AXES" :key="a.key" class="axis-chip">
                  <VIcon :icon="a.icon" size="14" class="me-1" :color="ratingInfo(it[a.key]).color" />
                  <span class="text-caption font-weight-medium">{{ a.label }}</span>
                  <VChip :color="ratingInfo(it[a.key]).color" variant="flat" size="x-small" class="ms-2">
              ...
              <VAlert v-if="it.guidance" type="info" variant="tonal" density="compact" class="mt-3" prepend-icon="ri-lightbulb-line">
                <span class="text-body-2">{{ it.guidance }}</span>
              </VAlert>
              <div v-if="it.notes" class="text-caption text-medium-emphasis mt-2">
                <VIcon icon="ri-sticky-note-line" size="12" class="me-1" />
                {{ it.notes }}
              </div>
            </VCardText>
            ...
            <VCardActions class="pa-2">
              <VSpacer />
              <VBtn variant="text" size="small" prepend-icon="ri-eye-line" @click="openDetails(it)">عرض / تعديل</VBtn>
            </VCardActions>
          </VCard>
        </VCol>
      </VRow>
    ...
    <VCard v-else rounded="lg" elevation="0" class="border">
      <VCardText>
        <div class="d-flex align-center mb-3">
          ...
          <VSpacer />
          <VChip color="primary" variant="flat" size="small">{{ Number(totalItems).toLocaleString() }} سجل</VChip>
        </div>
        <SmartTable
          ...
          @showItem="openDetails" @editItem="openDetails"
        />
      </VCardText>
    </VCard>
