...
export default {
  ...
  methods: {
    ...
    async submitDialog() {
      if (!this.validateForm()) { this.showAlert("error", "يرجى تصحيح الأخطاء في النموذج"); return; }
      this.saving = true;
      try {
  ...
  <div>
    ...
    <div class="expense-hero rounded-xl pa-5 pa-md-6 mb-6 d-flex flex-wrap align-center ga-3">
      <div class="hero-icon-wrap rounded-circle d-flex align-center justify-center">
        <VIcon icon="ri-wallet-3-line" size="28" color="white" />
      </div>
      <div class="flex-grow-1">
        <div class="text-h6 text-md-h5 font-weight-bold text-white">إدارة المصاريف</div>
    ...
    <VRow class="mb-4">
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="kpi-card kpi-primary">
          <VCardText>
            <div class="d-flex align-center justify-space-between mb-2">
              <VIcon icon="ri-money-dollar-circle-line" color="error" size="24" />
              <VChip size="small" color="error" variant="flat">إجمالي</VChip>
            </div>
            <div class="text-h5 font-weight-bold">{{ formatIQDShort(kpis.totalAmount) }}</div>
      ...
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="kpi-card">
          <VCardText>
            <div class="d-flex align-center justify-space-between mb-2">
              <VIcon icon="ri-list-check-2" color="info" size="24" />
              <VChip size="small" color="info" variant="flat">عدد</VChip>
            </div>
            <div class="text-h5 font-weight-bold">{{ numberWithComma(kpis.count) }}</div>
            <div class="text-caption text-medium-emphasis">عدد السجلات</div>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="kpi-card">
          <VCardText>
            <div class="d-flex align-center justify-space-between mb-2">
              <VIcon icon="ri-calculator-line" color="warning" size="24" />
              <VChip size="small" color="warning" variant="flat">متوسط</VChip>
            </div>
            <div class="text-h5 font-weight-bold">{{ formatIQDShort(kpis.averageAmount) }}</div>
            <div class="text-caption text-medium-emphasis">متوسط المصروف</div>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="kpi-card">
          <VCardText>
            <div class="d-flex align-center justify-space-between mb-2">
              <VIcon icon="ri-bar-chart-2-line" color="success" size="24" />
              <VChip size="small" :color="categoryColor(kpis.topCategory.value)" variant="flat">{{ topCategoryLabel }}</VChip>
            </div>
            <div class="text-h5 font-weight-bold">{{ formatIQDShort(kpis.topCategory.amount) }}</div>
            <div class="text-caption text-medium-emphasis">أعلى تصنيف</div>
          </VCardText>
        </VCard>
      </VCol>
    </VRow>
        ...
        <VRow dense>
          ...
          <VCol cols="6" md="2">
            <VSelect
              ...
              :items="categoryItems" item-title="title" item-value="value"
              label="التصنيف" density="comfortable" variant="outlined" clearable
              @update:model-value="onFilterChange"
            />
          </VCol>
          <VCol cols="6" md="2">
            <VSelect
              ...
              :items="paymentMethods" item-title="title" item-value="value"
              label="طريقة الدفع" density="comfortable" variant="outlined" clearable
              @update:model-value="onFilterChange"
            />
          </VCol>
          <VCol cols="6" md="3" class="d-flex align-center justify-end ga-2">
            <VSwitch
              v-model="filters.deleted" color="error" density="compact" hide-details
              label="عرض المحذوفة" @update:model-value="onFilterChange"
            />
            <VBtn color="primary" variant="tonal" icon="ri-search-2-line" size="small" @click="onSearch" />
          </VCol>
          <VCol cols="12" md="3">
            <VTextField v-model="filters.from" type="date" label="من تاريخ" density="comfortable" variant="outlined" clearable @update:model-value="onFilterChange" />
          </VCol>
          <VCol cols="12" md="3">
            <VTextField v-model="filters.to" type="date" label="إلى تاريخ" density="comfortable" variant="outlined" clearable @update:model-value="onFilterChange" />
          </VCol>
        </VRow>
      </VCardText>
    </VCard>
    ...
    <VCard rounded="lg" elevation="0" class="border">
      <VCardText>
        <div class="d-flex align-center mb-3">
          ...
          <VSpacer />
          <VChip color="primary" variant="flat" size="small">{{ numberWithComma(total) }} سجل</VChip>
        </div>
        <SmartTable
          :headers="headers" :items="items"
          :actions="tableActions" :loading="loadingTable"
          :totalItems="total" :tableOptions="tableOptions"
          @updateTableOptions="onUpdateTableOptions"
          @editItem="openEdit" @deleteItem="confirmDelete"
          @enableItem="confirmRestore" @showItem="viewExpenseDetails"
        />
      </VCardText>
    </VCard>

    <!-- Add/Edit Dialog -->
    <VDialog v-model="dialog.open" max-width="640" persistent>
      <VCard>
        <VCardTitle class="d-flex align-center py-4 px-6">
          <VIcon :icon="dialog.mode === 'add' ? 'ri-add-circle-line' : 'ri-pencil-line'" class="me-2" color="primary" />
          ...
          <VRow>
            <VCol cols="12" md="6">
              <VTextField
                ...
                label="المبلغ *" variant="outlined" density="comfortable"
            ...
            <VCol cols="12" md="6">
              <VTextField
                v-model="form.expense_date" type="date" label="تاريخ المصروف *"
            ...
            <VCol cols="12" md="6">
              <VSelect
                ...
                :items="categoryItems.filter(c => c.value)" item-title="title" item-value="value"
                label="التصنيف" variant="outlined" density="comfortable"
            ...
            <VCol cols="12" md="6">
              <VSelect
                ...
                :items="paymentMethods.filter(p => p.value)" item-title="title" item-value="value"
                label="طريقة الدفع" variant="outlined" density="comfortable"
                prepend-inner-icon="ri-bank-card-line"
              />
            </VCol>
            <VCol cols="12">
              <VTextarea v-model="form.note" label="ملاحظة" variant="outlined" rows="2" prepend-inner-icon="ri-sticky-note-line" />
            </VCol>
          </VRow>

          <VCard v-if="dialog.mode === 'edit'" variant="tonal" class="mt-2">
        ...
        <VCardActions class="px-6 py-3">
          <VSpacer />
          <VBtn variant="text" @click="closeDialog">إلغاء</VBtn>
          <VBtn color="primary" :loading="saving" prepend-icon="ri-save-3-line" @click="submitDialog">حفظ</VBtn>
        </VCardActions>
      </VCard>
    </VDialog>

    <!-- View Dialog -->
    <VDialog v-model="viewDialog.open" max-width="560">
      <VCard>
        <VCardTitle class="d-flex align-center py-4 px-6">
          <VIcon icon="ri-eye-line" class="me-2" color="primary" />
        ...
        <VCardText>
          <VRow>
            <VCol cols="12" md="6">
              ...
              <div class="text-h6 font-weight-bold text-error">{{ formatIQD(viewDialog.data.amount) }}</div>
            </VCol>
            <VCol cols="12" md="6">
            ...
            <VCol cols="12" md="6">
              ...
              <VChip size="small" :color="categoryColor(viewDialog.data.category)" variant="flat">
                {{ categoryLabel(viewDialog.data.category) }}
              </VChip>
            </VCol>
            <VCol cols="12" md="6">
              <div class="text-caption text-medium-emphasis">طريقة الدفع</div>
              <div class="text-body-1">{{ paymentMethodLabel(viewDialog.data.payment_method) }}</div>
            </VCol>
            <VCol cols="12">
              <div class="text-caption text-medium-emphasis">ملاحظة</div>
              <div class="text-body-1">{{ viewDialog.data.note || "—" }}</div>
            </VCol>
        ...
        <VCardActions class="px-6 py-3">
          <VSpacer />
          <VBtn variant="text" @click="viewDialog.open = false">إغلاق</VBtn>
        </VCardActions>
      </VCard>
    </VDialog>
    ...
    />
    <ConfirmDangerDialog
      v-if="restoreDialog.open" v-model="restoreDialog.open"
