...
export default {
  ...
  created() {
    ...
    if (stateInv && typeof stateInv === "object") {
      const fields = ["id", "student_name", "course_name", "amount_due", "discount_total",
                      "amount_paid", "remaining_amount", "invoice_status", "study_year",
                      "payment_mode", "invoice_date", "due_date", "notes"];
...
<template>
  <div>
    ...
    <div class="inv-hero rounded-xl pa-5 pa-md-6 mb-6">
      <div class="d-flex flex-wrap align-center ga-3">
        <div class="hero-icon-wrap rounded-circle d-flex align-center justify-center">
          <VIcon icon="ri-bill-line" size="28" color="white" />
        </div>
        <div class="flex-grow-1">
          <div class="d-flex align-center ga-2 flex-wrap">
            <span class="text-h6 text-md-h5 font-weight-bold text-white">{{ inv.student_name || "فاتورة طالب" }}</span>
            <VChip v-if="inv.invoice_status" :color="statusInfo.color" size="small" variant="flat" :prepend-icon="statusInfo.icon" class="text-white">
              {{ statusInfo.label }}
            </VChip>
            <VChip v-if="isDeleted" color="error" size="small" variant="flat" prepend-icon="ri-delete-bin-line" class="text-white">
              محذوفة
            </VChip>
          </div>
        ...
        <div class="d-flex ga-2">
          <RouterLink :to="{ name: 'teacher-invoices-manage-invoices' }" class="text-decoration-none">
            <VBtn variant="flat" color="white" class="text-primary" prepend-icon="ri-arrow-right-line">رجوع</VBtn>
          </RouterLink>
          <VBtn v-if="!isDeleted && Number(inv.remaining_amount) > 0" color="warning" prepend-icon="ri-money-dollar-circle-line" @click="payInvoice">دفعة</VBtn>
          <VBtn v-if="!isDeleted" color="error" variant="tonal" prepend-icon="ri-delete-bin-line" @click="openDeleteDialog">حذف</VBtn>
          <VBtn v-else color="success" variant="tonal" prepend-icon="ri-refresh-line" @click="openRestoreDialog">استرجاع</VBtn>
        </div>
      </div>
      ...
      <div class="mt-4">
        ...
        <VProgressLinear :model-value="paidPct" color="warning" bg-color="rgba(255,255,255,0.2)" height="8" rounded />
      </div>
    </div>
    ...
    <VRow class="mb-4">
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="border">
          <VCardText>
            <div class="text-caption text-medium-emphasis">المبلغ المستحق</div>
            <div class="text-h6 font-weight-bold mt-1">{{ formatIQD(inv.amount_due) }}</div>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="border">
          <VCardText>
            <div class="text-caption text-medium-emphasis">الخصم</div>
            <div class="text-h6 font-weight-bold text-info mt-1">{{ formatIQD(inv.discount_total) }}</div>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="border">
          <VCardText>
            ...
            <div class="text-h6 font-weight-bold text-success mt-1">{{ formatIQD(inv.amount_paid) }}</div>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="6" md="3">
        <VCard rounded="lg" elevation="0" class="border">
          <VCardText>
            ...
            <div class="text-h6 font-weight-bold text-error mt-1">{{ formatIQD(inv.remaining_amount) }}</div>
          </VCardText>
        </VCard>
      </VCol>
    </VRow>
    ...
    <VRow class="mb-4">
      <VCol cols="12" md="6">
        <VCard rounded="lg" elevation="0" class="border h-100">
          <VCardText>
            <div class="text-subtitle-2 font-weight-bold mb-2"><VIcon icon="ri-information-line" class="me-1" /> بيانات الفاتورة</div>
            <VRow>
              <VCol cols="6"><div class="text-caption text-medium-emphasis">تاريخ الفاتورة</div><div>{{ formatDate(inv.invoice_date) }}</div></VCol>
              <VCol cols="6"><div class="text-caption text-medium-emphasis">تاريخ الاستحقاق</div><div>{{ formatDate(inv.due_date) }}</div></VCol>
              <VCol cols="12"><div class="text-caption text-medium-emphasis">ملاحظات</div><div>{{ inv.notes || "—" }}</div></VCol>
            </VRow>
          </VCardText>
        </VCard>
      </VCol>
      <VCol cols="12" md="6">
        <VCard rounded="lg" elevation="0" class="border h-100">
          <VCardText>
            <div class="text-subtitle-2 font-weight-bold mb-2"><VIcon icon="ri-pie-chart-2-line" class="me-1" /> إحصائيات الأقساط</div>
            <VRow dense>
              <VCol cols="4" sm="2"><VChip size="small" color="primary" variant="flat" class="w-100">العدد: {{ totals.count }}</VChip></VCol>
              <VCol cols="4" sm="2"><VChip size="small" color="success" variant="flat" class="w-100">مدفوع: {{ totals.paidCount }}</VChip></VCol>
              <VCol cols="4" sm="2"><VChip size="small" color="info" variant="flat" class="w-100">جزئي: {{ totals.partialCount }}</VChip></VCol>
              <VCol cols="4" sm="2"><VChip size="small" color="warning" variant="flat" class="w-100">معلق: {{ totals.pendingCount }}</VChip></VCol>
              <VCol cols="4" sm="2"><VChip size="small" color="error" variant="flat" class="w-100">متأخر: {{ totals.overdueCount }}</VChip></VCol>
            </VRow>
          </VCardText>
        </VCard>
      </VCol>
    ...
    <VCard rounded="lg" elevation="0" class="border mb-4">
      <VCardText>
        ...
        <VAlert v-if="loading" type="info" variant="tonal" density="compact">جاري التحميل...</VAlert>
        <VAlert v-else-if="!installments.length" type="info" variant="tonal" density="compact">
        ...
        <VTable v-else density="comfortable">
          ...
          <tbody>
            <tr v-for="ins in installments" :key="ins.id">
              ...
              <td :data-label="'المتبقّي'">{{ numberWithComma(ins.remaining_amount) }}</td>
              <td :data-label="'تاريخ الاستحقاق'">{{ formatDate(ins.due_date) }}</td>
              <td :data-label="'الحالة'">
                <VChip :color="installmentStatusInfo(ins.installment_status).color" size="small" variant="flat" :prepend-icon="installmentStatusInfo(ins.installment_status).icon">
                  {{ installmentStatusInfo(ins.installment_status).label }}
                </VChip>
              </td>
              <td :data-label="'العمليات'" class="text-end">
                <VBtn
                  v-if="!isDeleted && ins.installment_status !== 'paid' && Number(ins.remaining_amount) > 0"
                  size="small" color="primary" variant="tonal" prepend-icon="ri-money-dollar-circle-line"
    ...
    <!-- Add Payment Dialog -->
    <VDialog v-model="payDialog.open" max-width="520">
      <VCard>
        ...
        <VDivider />
        <VCardText>
          <VAlert v-if="payDialog.installmentId" type="info" variant="tonal" density="compact" class="mb-3">
          ...
          <VRow>
            <VCol cols="12">
              <VTextField v-model.number="payDialog.form.amount" type="number" label="المبلغ" variant="outlined" density="comfortable" prepend-inner-icon="ri-money-dollar-circle-line" />
            </VCol>
            <VCol cols="12" md="6">
              <VSelect v-model="payDialog.form.paymentMethod" :items="paymentMethods" item-title="text" item-value="value" label="طريقة الدفع" variant="outlined" density="comfortable" prepend-inner-icon="ri-bank-card-line" />
            </VCol>
            <VCol cols="12" md="6">
              <VTextField v-model="payDialog.form.paidAt" type="datetime-local" label="تاريخ الدفع" variant="outlined" density="comfortable" />
            </VCol>
            <VCol cols="12">
              <VTextField v-model="payDialog.form.notes" label="ملاحظات" variant="outlined" density="comfortable" prepend-inner-icon="ri-sticky-note-line" />
            </VCol>
          </VRow>
        </VCardText>
        <VDivider />
        <VCardActions>
          <VSpacer />
          <VBtn variant="text" @click="payDialog.open = false">إلغاء</VBtn>
          <VBtn color="primary" :loading="saving" prepend-icon="ri-check-line" @click="submitPayment">تسديد</VBtn>
        </VCardActions>
      </VCard>
    </VDialog>

    <!-- Delete -->
    <VDialog v-model="deleteDialog.open" max-width="520">
      <VCard>
        <VCardTitle class="d-flex align-center"><VIcon icon="ri-delete-bin-line" class="me-2" /> تأكيد الحذف</VCardTitle>
        <VCardText>سيتم حذف الفاتورة حذفاً منطقياً مع أقساطها. يمكن استرجاعها لاحقاً.</VCardText>
        <VCardActions>
          <VSpacer />
          <VBtn variant="text" @click="deleteDialog.open = false">إلغاء</VBtn>
          <VBtn color="error" :loading="deleteDialog.loading" @click="handleSoftDelete">حذف</VBtn>
        </VCardActions>
      </VCard>
    </VDialog>

    <!-- Restore -->
    <VDialog v-model="restoreDialog.open" max-width="520">
      <VCard>
        <VCardTitle class="d-flex align-center"><VIcon icon="ri-refresh-line" class="me-2" /> تأكيد الاسترجاع</VCardTitle>
        <VCardText>سيتم استرجاع الفاتورة مع أقساطها.</VCardText>
        <VCardActions>
          <VSpacer />
          <VBtn variant="text" @click="restoreDialog.open = false">إلغاء</VBtn>
          <VBtn color="success" :loading="restoreDialog.loading" @click="handleRestore">استرجاع</VBtn>
        </VCardActions>
      </VCard>
    </VDialog>
