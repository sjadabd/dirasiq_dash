...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      if (err) { this.showAlert("error", err); return; }
  ...
  <div>
    ...
    <div class="invoice-create-hero rounded-xl pa-5 pa-md-6 mb-6 d-flex flex-wrap align-center ga-3">
      <div class="hero-icon-wrap rounded-circle d-flex align-center justify-center">
        <VIcon icon="ri-file-add-line" size="28" color="white" />
      </div>
      <div class="flex-grow-1">
        <div class="text-h6 text-md-h5 font-weight-bold text-white">إنشاء فاتورة جديدة</div>
        ...
        <VRow>
          <VCol cols="12" md="4">
            <VSelect
              ...
              :loading="coursesLoading" :disabled="coursesLoading"
              prepend-inner-icon="ri-book-3-line" clearable
          ...
          <VCol cols="12" md="4">
            <VSelect
              ...
              :items="studentItems" item-title="name" item-value="id"
              label="اختر الطالب" variant="outlined" density="comfortable"
              :loading="studentsLoading" :disabled="studentsLoading || !form.courseId"
              prepend-inner-icon="ri-user-3-line" clearable
            />
          </VCol>
          <VCol cols="12" md="4">
            <VSelect
        ...
        <VRow>
          <VCol cols="12" md="3">
            <VTextField
              ...
              label="المبلغ المستحق (د.ع)" variant="outlined" density="comfortable"
          ...
          <VCol cols="12" md="3">
            <VTextField
              ...
              label="خصم عام (د.ع)" variant="outlined" density="comfortable"
          ...
          <VCol cols="12" md="3">
            <VTextField v-model="form.invoiceDate" type="date" label="تاريخ الفاتورة" variant="outlined" density="comfortable" />
          </VCol>
          <VCol cols="12" md="3">
            <VTextField v-model="form.dueDate" type="date" label="تاريخ الاستحقاق (اختياري)" variant="outlined" density="comfortable" />
          </VCol>
          <VCol cols="12">
            <VTextField v-model="form.notes" label="ملاحظات" variant="outlined" density="comfortable" prepend-inner-icon="ri-sticky-note-line" />
          </VCol>
        </VRow>
        <VAlert v-if="form.amountDue" type="info" variant="tonal" density="compact" class="mt-2">
    ...
    <VCard rounded="lg" elevation="0" class="mb-4 border">
      <VCardText>
        ...
        <VRow>
          <VCol cols="12" md="6">
            <VSelect
              ...
              :items="paymentModes" item-title="text" item-value="value"
              label="طريقة الدفع" variant="outlined" density="comfortable"
              prepend-inner-icon="ri-bank-card-line"
            />
          </VCol>
          <VCol v-if="form.paymentMode === 'installments' && !manualMode" cols="12" md="2">
            <VTextField v-model.number="autoSplit.count" type="number" min="2" max="36" label="عدد الأقساط" variant="outlined" density="comfortable" />
          </VCol>
          <VCol v-if="form.paymentMode === 'installments' && !manualMode" cols="12" md="2">
            <VTextField v-model.number="autoSplit.intervalDays" type="number" min="1" max="365" label="الفترة (أيام)" variant="outlined" density="comfortable" />
          </VCol>
          <VCol v-if="form.paymentMode === 'installments' && !manualMode" cols="12" md="2">
            <VTextField v-model="autoSplit.firstDueDate" type="date" label="أول قسط" variant="outlined" density="comfortable" />
          </VCol>
        </VRow>

        <!-- Auto-split preview -->
        <VAlert v-if="form.paymentMode === 'installments' && !manualMode && netAmount > 0" type="success" variant="tonal" density="compact" class="mt-2">
        ...
        <!-- Advanced toggle -->
        <div v-if="form.paymentMode === 'installments'" class="mt-3">
          <VBtn variant="text" size="small" :prepend-icon="manualMode ? 'ri-arrow-up-s-line' : 'ri-arrow-down-s-line'" @click="toggleManual">
            {{ manualMode ? "إخفاء الوضع المتقدّم (إدارة يدوية لكل قسط)" : "وضع متقدّم (مبالغ مختلفة لكل قسط)" }}
          </VBtn>
        </div>

        <!-- Manual installments table -->
        <div v-if="form.paymentMode === 'installments' && manualMode" class="mt-3">
          <VAlert v-if="manualInstallments.length" :type="Math.abs(manualDelta) > 0.5 ? 'warning' : 'success'" variant="tonal" density="compact" class="mb-3">
            <div class="d-flex flex-wrap align-center justify-space-between ga-2">
              ...
              <span v-if="Math.abs(manualDelta) > 0.5">
                <VChip size="small" :color="manualDelta > 0 ? 'warning' : 'error'" variant="flat">
                  الفرق: {{ formatMoney(manualDelta) }}
                </VChip>
              </span>
              <VChip v-else size="small" color="success" variant="flat">مطابق</VChip>
            </div>
          </VAlert>

          <VRow v-for="(ins, idx) in manualInstallments" :key="idx" class="mb-1">
            <VCol cols="12" md="1">
              <VTextField v-model.number="ins.installmentNumber" type="number" label="#" variant="outlined" density="comfortable" />
            </VCol>
            <VCol cols="12" md="3">
              <VTextField
            ...
            <VCol cols="12" md="3">
              <VTextField v-model="ins.dueDate" type="date" label="تاريخ الاستحقاق" variant="outlined" density="comfortable" />
            </VCol>
            <VCol cols="12" md="4">
              <VTextField v-model="ins.notes" label="ملاحظات" variant="outlined" density="comfortable" />
            </VCol>
            <VCol cols="12" md="1" class="d-flex align-center">
              <VBtn color="error" size="small" icon="ri-delete-bin-line" variant="text" @click="removeManualRow(idx)" />
            </VCol>
          </VRow>

          <div class="d-flex ga-2">
            <VBtn variant="tonal" size="small" prepend-icon="ri-add-line" @click="addManualRow">إضافة قسط</VBtn>
            <VBtn variant="text" size="small" prepend-icon="ri-magic-line" @click="seedManualFromAuto">توليد تلقائي</VBtn>
          </div>
        </div>
      </VCardText>
    </VCard>
