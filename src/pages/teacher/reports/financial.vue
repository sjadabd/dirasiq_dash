  ...
  <div>
    ...
    <div class="report-hero rounded-xl pa-5 pa-md-6 mb-6 d-flex flex-wrap align-center ga-3">
      <div class="hero-icon-wrap rounded-circle d-flex align-center justify-center">
        <VIcon icon="ri-bar-chart-2-line" size="28" color="white" />
      </div>
      <div class="flex-grow-1">
        <div class="text-h6 text-md-h5 font-weight-bold text-white">التقرير المالي</div>
      ...
      <div class="d-flex ga-2">
        <VBtn variant="flat" color="white" class="text-primary" prepend-icon="ri-refresh-line" :loading="loading" @click="fetchReport">تحديث</VBtn>
        <VBtn color="success" prepend-icon="ri-file-excel-2-line" :loading="exporting.excel" @click="exportToExcel">Excel</VBtn>
        <VBtn color="error" prepend-icon="ri-file-pdf-2-line" :loading="exporting.pdf" @click="exportToPDF">PDF</VBtn>
      </div>
    </div>
    ...
    <VCard rounded="lg" elevation="0" class="mb-4 border">
      <VCardText>
        <div class="d-flex align-center mb-3 ga-2">
          ...
          <VSpacer />
          <VBtn variant="text" size="small" prepend-icon="ri-refresh-line" @click="resetFilters">إعادة تعيين</VBtn>
        </div>
        <VRow dense>
          <VCol cols="12" md="4">
            <VSelect
              ...
              label="السنة الدراسية" variant="outlined" density="comfortable"
              :loading="yearsLoading" prepend-inner-icon="ri-calendar-2-line"
              @update:model-value="fetchReport"
            />
          </VCol>
          <VCol cols="12" md="4">
            <VTextField v-model="filters.from" type="date" label="من تاريخ" variant="outlined" density="comfortable" clearable prepend-inner-icon="ri-calendar-line" @update:model-value="fetchReport" />
          </VCol>
          <VCol cols="12" md="4">
            <VTextField v-model="filters.to" type="date" label="إلى تاريخ" variant="outlined" density="comfortable" clearable prepend-inner-icon="ri-calendar-line" @update:model-value="fetchReport" />
          </VCol>
        </VRow>
      </VCardText>
    </VCard>
    ...
    <div ref="printArea">
          ...
          <VCard rounded="lg" elevation="0" class="kpi-card">
            <VCardText>
              <div class="d-flex align-center justify-space-between mb-2">
                <VIcon icon="ri-money-dollar-circle-line" color="success" size="24" />
                <VChip size="small" color="success" variant="flat">مقبوض</VChip>
              </div>
              <div class="text-h5 font-weight-bold">{{ formatShort(summary.totalPaidIncome) }}</div>
              <div class="text-caption text-medium-emphasis">الدخل المقبوض</div>
            </VCardText>
          </VCard>
        </VCol>
        <VCol cols="6" md="3">
          <VCard rounded="lg" elevation="0" class="kpi-card">
            <VCardText>
              <div class="d-flex align-center justify-space-between mb-2">
                <VIcon icon="ri-calendar-check-line" color="info" size="24" />
                <VChip size="small" color="info" variant="flat">{{ collectionRatePct }}% تحصيل</VChip>
              </div>
              <div class="text-h5 font-weight-bold">{{ formatShort(summary.totalDueIncome) }}</div>
              <div class="text-caption text-medium-emphasis">إجمالي المستحقات</div>
            </VCardText>
          </VCard>
        </VCol>
        <VCol cols="6" md="3">
          <VCard rounded="lg" elevation="0" class="kpi-card">
            <VCardText>
              <div class="d-flex align-center justify-space-between mb-2">
                <VIcon icon="ri-shopping-cart-line" color="error" size="24" />
                <VChip size="small" color="error" variant="flat">مصاريف</VChip>
              </div>
              <div class="text-h5 font-weight-bold">{{ formatShort(expenses.total) }}</div>
              <div class="text-caption text-medium-emphasis">إجمالي المصاريف</div>
            </VCardText>
          </VCard>
        </VCol>
        <VCol cols="6" md="3">
          <VCard rounded="lg" elevation="0" class="kpi-card kpi-profit">
            <VCardText>
              <div class="d-flex align-center justify-space-between mb-2">
                <VIcon icon="ri-line-chart-line" color="warning" size="24" />
                <VChip size="small" :color="profitMarginPct >= 0 ? 'warning' : 'error'" variant="flat">{{ profitMarginPct }}% هامش</VChip>
              </div>
              <div class="text-h5 font-weight-bold" :class="summary.netProfitPaidBasis >= 0 ? 'text-warning' : 'text-error'">
                {{ formatShort(summary.netProfitPaidBasis) }}
              </div>
              <div class="text-caption text-medium-emphasis">صافي الربح (مقبوض)</div>
            </VCardText>
          </VCard>
        </VCol>
      </VRow>
          ...
          <VCard rounded="lg" elevation="0" class="border">
            <VCardText>
              <div class="d-flex align-center mb-3">
                <VIcon icon="ri-bar-chart-box-line" color="primary" class="me-2" />
                <span class="text-subtitle-1 font-weight-bold">مقارنة الدخل والمصاريف</span>
              </div>
              <apexchart type="bar" height="320" :options="comparisonBarOptions" :series="comparisonBarSeries" />
            </VCardText>
          </VCard>
        </VCol>
        <VCol cols="12" md="4">
          <VCard rounded="lg" elevation="0" class="border h-100">
            <VCardText>
              <div class="d-flex align-center mb-3">
                <VIcon icon="ri-pie-chart-line" color="primary" class="me-2" />
                <span class="text-subtitle-1 font-weight-bold">المصاريف حسب التصنيف</span>
              </div>
              <apexchart type="donut" height="320" :options="expenseDonutOptions" :series="expenseDonutSeries" />
            </VCardText>
          </VCard>
        </VCol>
      </VRow>
      ...
      <VCard rounded="lg" elevation="0" class="border mb-4">
        <VCardText>
          <div class="d-flex align-center mb-3">
            <VIcon icon="ri-bill-line" color="primary" class="me-2" />
            <span class="text-subtitle-1 font-weight-bold">تفصيل الفواتير: مدفوع / متبقّي / خصم</span>
          </div>
          <apexchart type="bar" height="260" :options="invoiceSplitOptions" :series="invoiceSplitSeries" />
        </VCardText>
      </VCard>
      ...
      <VRow class="mb-4">
        <VCol cols="12" md="6">
          <VCard rounded="lg" elevation="0" class="border h-100">
            <VCardText>
              <div class="d-flex align-center mb-3">
                <VIcon icon="ri-group-line" color="info" class="me-2" />
        ...
        <VCol cols="12" md="6">
          <VCard rounded="lg" elevation="0" class="border h-100">
            <VCardText>
              <div class="d-flex align-center mb-3">
                <VIcon icon="ri-shopping-bag-line" color="warning" class="me-2" />
      ...
      <VCard rounded="lg" elevation="0" class="border mb-4">
        <VCardText>
          <div class="d-flex align-center mb-3">
            <VIcon icon="ri-funds-line" color="primary" class="me-2" />
            <span class="text-subtitle-1 font-weight-bold">الملخص النهائي</span>
          </div>
          <VRow>
            <VCol cols="12" sm="6" md="3">
              <div class="text-caption text-medium-emphasis">الدخل المقبوض</div>
              <div class="text-h6 font-weight-bold text-success">{{ formatIQD(summary.totalPaidIncome) }}</div>
            </VCol>
            <VCol cols="12" sm="6" md="3">
              <div class="text-caption text-medium-emphasis">الدخل المستحق</div>
              <div class="text-h6 font-weight-bold text-info">{{ formatIQD(summary.totalDueIncome) }}</div>
            </VCol>
            <VCol cols="12" sm="6" md="3">
              <div class="text-caption text-medium-emphasis">صافي الربح (مقبوض)</div>
              <div class="text-h6 font-weight-bold" :class="summary.netProfitPaidBasis >= 0 ? 'text-warning' : 'text-error'">
                {{ formatIQD(summary.netProfitPaidBasis) }}
              </div>
            </VCol>
            <VCol cols="12" sm="6" md="3">
              <div class="text-caption text-medium-emphasis">صافي الربح (استحقاق)</div>
              <div class="text-h6 font-weight-bold" :class="summary.netProfitDueBasis >= 0 ? 'text-primary' : 'text-error'">
