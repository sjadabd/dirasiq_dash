...
export default {
  ...
  methods: {
    ...
    async loadStudentsByCourse(courseId) {
      if (!courseId) { this.studentsOptions = []; this.createDialog.form.recipients.selectedStudentIds = [] 
    ...
    async loadStudentsBySession(sessionId) {
      if (!sessionId) { this.studentsOptions = []; this.createDialog.form.recipients.selectedStudentIds = [] 
    ...
    async onPdfSelected(e) {
      ...
      if (!file) { this.createDialog.form.attachments.pdfBase64 = null 
    ...
    async submitCreateNotification() {
      ...
      if (!f.title?.trim()) { this.showAlert("error", "العنوان مطلوب") 

        return }
      if (!f.message?.trim()) { this.showAlert("error", "نص الرسالة مطلوب") 

        return }
      if (!f.recipients?.mode) { this.showAlert("error", "اختر نطاق المستلمين") 
      ...
      if (mode === "specific_students" || mode === "students_of_course" || mode === "students_of_session") {
        ...
        if (!ids.length) { this.showAlert("error", "اختر طالباً واحداً على الأقل") 
  ...
  <div>
    ...
    >
      ...
      <div class="text-body-2 text-medium-emphasis mb-4">
        ابدأ بإرسال إشعارك الأول لطلابك.
      </div>
      <VBtn
        color="primary"
        prepend-icon="ri-add-line"
    ...
    <VRow v-else>
      ...
      >
        <VCard
          rounded="lg"
          elevation="0"
          class="notif-card h-100 d-flex flex-column"
        >
          <VCardText class="flex-grow-1">
            <!-- Header -->
            <div class="d-flex align-start ga-3 mb-3">
              <VAvatar
                :color="typeInfo(it.type).color"
                size="40"
                rounded
              >
                <VIcon
                  :icon="subTypeInfo(it.data?.subType)?.icon || 'ri-notification-3-line'"
                  color="white"
                />
              </VAvatar>
              <div class="flex-grow-1 overflow-hidden">
                <div class="d-flex align-center ga-2 flex-wrap">
                  ...
                  <VChip
                    v-if="subTypeInfo(it.data?.subType)"
                    :color="subTypeInfo(it.data?.subType).color"
                    size="x-small"
                    variant="flat"
                  >
            ...
            <!-- Image attachments -->
            <div
              v-if="attachmentImages(it).length"
              class="d-flex flex-wrap ga-2 mb-3"
            >
              <VImg
                v-for="(url, idx) in attachmentImages(it)"
                :key="idx"
                :src="url"
                :width="80"
                :height="80"
        ...
        <VCardText style="max-height: 75vh;">
            ...
            <VRow dense>
              ...
              >
                <VSelect
                  ...
                  :items="SUB_TYPES.filter(s => s.value)"
                  item-title="text"
                  item-value="value"
                  label="نوع الإشعار"
                  variant="outlined"
              ...
              <VCol cols="12">
                <VTextarea
                  v-model="createDialog.form.message"
                  label="نص الرسالة *"
                  variant="outlined"
                  rows="4"
                  auto-grow
              ...
              >
                <VTextField
                  v-model="createDialog.form.link"
                  label="رابط (اختياري)"
                  variant="outlined"
                  density="comfortable"
          ...
          <div class="dialog-section mb-4">
            ...
            <VRow dense>
              ...
              <VCol
                v-if="createDialog.form.recipients.mode === 'students_of_course'"
                cols="12"
                md="6"
              >
                <VSelect
              ...
              <VCol
                v-if="createDialog.form.recipients.mode === 'students_of_session'"
                cols="12"
                md="6"
              >
                <VSelect
              ...
              <VCol
                v-if="createDialog.form.recipients.mode !== 'all_students_of_teacher'"
                cols="12"
                :md="createDialog.form.recipients.mode === 'specific_students' ? 12 : 6"
              >
                <VSelect
                ...
                >
                  <template #prepend-item>
                    <VListItem @click="selectedAllStudents = !selectedAllStudents">
                      <template #prepend>
                        <VCheckbox
                          :model-value="selectedAllStudents"
                          hide-details
                        />
                      </template>
                      <VListItemTitle class="font-weight-bold">
                        تحديد الكل ({{ studentsOptions.length }})
                      </VListItemTitle>
                    </VListItem>
                    <VDivider />
                  </template>
          ...
          <div class="dialog-section">
            ...
            <VRow dense>
              ...
              >
                ...
                />
                <VChip
                  v-if="createDialog.form.attachments.pdfBase64"
                  color="error"
                  variant="tonal"
                  size="small"
                  closable
                  class="mt-1"
                  @click:close="removePdfAttachment"
                >
                  مرفق PDF جاهز
                </VChip>
              </VCol>
              ...
              >
                ...
                />
                <div
                  v-if="createDialog.form.attachments.imagesBase64.length"
                  class="d-flex flex-wrap ga-1 mt-1"
                >
                  <VChip
                    v-for="(_, idx) in createDialog.form.attachments.imagesBase64"
                    :key="idx"
                    color="info"
                    variant="tonal"
                    size="small"
