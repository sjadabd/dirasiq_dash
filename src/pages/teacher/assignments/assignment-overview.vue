                ...
                <VRow>
                  ...
                  >
                    ...
                    <div class="text-body-2">
                      {{ formatDateTime(assignment?.assigned_date) || '—' }}
                    </div>
                  </VCol>
                  <VCol
                    cols="12"
                  ...
                  <VCol cols="12">
                    ...
                    <VRow
                      v-if="Array.isArray(assignment?.attachments?.files) && assignment.attachments.files.length"
                      class="ga-2"
                    >
                      <VCol
                        v-for="(f, i) in assignment.attachments.files"
                        :key="i"
                        cols="12"
                        sm="6"
                        md="4"
                      >
                        <div v-if="(f.type || '').toLowerCase() === 'image'">
                          <VImg
                            ...
                            @click="openImage({ url: f.url, name: f.name })"
                          />
                        </div>
                        <div v-else>
                          <VBtn
                  ...
                  <VCol
                    v-if="Array.isArray(assignment?.resources) && assignment.resources.length"
                    cols="12"
                  >
                    <div class="text-subtitle-2 mb-2">
                      الموارد
                    ...
                    <div class="d-flex flex-wrap ga-2">
                      <VBtn
                        v-for="(r, idx) in assignment.resources"
                        :key="idx"
                        :href="r.url"
                        target="_blank"
                        rel="noopener"
                  ...
                  >
                    <VCard>
                      <VCardText>
                        ...
                        <div
                          v-if="Array.isArray(sub.attachments) && sub.attachments.length"
                          class="mb-2"
                        >
                          <strong class="d-block mb-1">المرفقات:</strong>
                          <VRow class="ga-2">
                            <VCol
                              v-for="(att, i) in sub.attachments"
                              :key="i"
                              cols="12"
                              sm="6"
                              md="4"
                        ...
                        <div class="d-flex align-center ga-3">
                          <div>
                            <strong>الدرجة:</strong> {{ sub.score ?? '—' }} / {{ assignment?.max_score ?? '—' }}
                          </div>
...
export default {
  name: 'TeacherAssignmentsAssignmentOverview',
  data() {
    return {
      breadcrumbItems: [
