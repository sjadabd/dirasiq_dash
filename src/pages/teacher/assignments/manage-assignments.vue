    ...
    >
      ...
      <VCardItem>
        <SmartTable
          ...
          @update-response-item="openGradeDialog"
          @send-notifications-item="openRecipientsDialog"
          @show-item="openAssignmentOverview"
        />
      </VCardItem>
    </VCard>
          ...
          <VRow>
            ...
            >
              <VSelect
                ...
                label="نوع التسليم (مطلوب للطالب)"
                variant="outlined"
              />
            </VCol>
            <VCol
            ...
            >
              <VSelect
                ...
                :items="deliveryModes"
                label="وضع التسليم (ورقي/إلكتروني)"
                variant="outlined"
              />
            </VCol>
            ...
            <VCol
              v-if="form.visibility === 'specific_students'"
              cols="12"
            >
              <VSelect
            ...
            >
              ...
              <VRow
                v-for="(ex, i) in form.attachments.existing"
                :key="i"
                class="mb-2"
              >
                <VCol
            ...
            <VCol cols="12">
              ...
              <VRow
                v-for="(resItem, idx) in form.resources"
                :key="idx"
                class="mb-2"
              >
                <VCol
      ...
      <VCard>
        ...
        <VCardText>
          <VRow>
            ...
            >
              <VTextField
                v-model.number="gradeDialog.score"
                type="number"
                label="الدرجة"
              />
            </VCol>
            ...
            >
              <VTextField
                v-model="gradeDialog.feedback"
                label="ملاحظات (اختياري)"
              />
            </VCol>
            <!-- Submission preview -->
            <VCol
              v-if="gradeDialog.submission"
              cols="12"
            >
              <VCard
                variant="tonal"
              ...
              >
                <div class="d-flex align-center mb-2">
                  <VChip
                    size="small"
                    :color="gradeDialog.submission.status === 'submitted' ? 'primary' : 'default'"
                    class="me-2"
                  >
                ...
                <div
                  v-if="gradeDialog.submission.content_text"
                  class="mb-2"
                >
                  <strong>النص:</strong>
                  <div
                  ...
                  >
                    {{ gradeDialog.submission.content_text }}
                  </div>
                </div>
                <div
                  v-if="gradeDialog.submission.link_url"
                  class="mb-2"
                >
                  <strong>رابط:</strong>
                  <a
                    :href="gradeDialog.submission.link_url"
                    target="_blank"
                    rel="noopener"
                    class="ms-1"
                  >فتح الرابط</a>
                ...
                <div v-if="Array.isArray(gradeDialog.submission.attachments) && gradeDialog.submission.attachments.length">
                  ...
                  <VRow class="ga-2">
                    <VCol
                      v-for="(att, i) in gradeDialog.submission.attachments"
                      :key="i"
                      cols="12"
                      sm="6"
                    >
      ...
      <VCard>
        <VCardTitle class="d-flex align-center">
          ...
          <VBtn
            icon
            variant="text"
            @click="recipientsDialog.open = false"
          >
            <VIcon>ri-close-line</VIcon>
          </VBtn>
        </VCardTitle>
        ...
        <VCardActions>
          ...
          <VBtn
            color="primary"
            :loading="recipientsDialog.loading"
...
export default {
  name: 'TeacherAssignmentsManageAssignments',
  data() {
    return {
      breadcrumbItems: [
    ...
    'gradeDialog.studentId': {
      async handler(sid) {
        try {
          ...
          if (!aId || !sid) { this.gradeDialog.score = null; this.gradeDialog.feedback = ''; this.gradeDialog.submission = null 
  ...
  methods: {
    ...
    async onPdfSelected(files) {
      try {
        ...
        if (!file) { this.form.attachments.pdf = { name: '', base64: '' } 
    ...
    async submitGrade() {
      try {
        ...
        if (!assignmentId || !studentId) { this.showAlert('error', 'اختر الطالب') 
    ...
    async loadStudentsByCourse(courseId) {
      try {
        if (!courseId) { this.studentsOptions = [] 
    ...
    async loadStudentsBySession(sessionId) {
      try {
        if (!sessionId) { this.studentsOptions = [] 
