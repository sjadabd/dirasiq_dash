  ...
  <div>
    ...
    <VRow class="my-2" dense>
      <VCol cols="12" sm="6" lg="3">
        <VCard class="stat-card" elevation="0" rounded="lg" border>
          <VCardItem>
            <div class="d-flex align-center">
              <VAvatar color="primary" size="44" class="me-3">
                <VIcon color="white">ri-notification-3-line</VIcon>
              </VAvatar>
              <div>
                <div class="stat-label">إجمالي الإشعارات</div>
                <div class="stat-value">{{ statsLoading ? "…" : (stats.total ?? 0) }}</div>
              </div>
            </div>
          </VCardItem>
        </VCard>
      ...
      <VCol cols="12" sm="6" lg="3">
        <VCard class="stat-card" elevation="0" rounded="lg" border>
          <VCardItem>
            <div class="d-flex align-center">
              <VAvatar color="success" size="44" class="me-3">
                <VIcon color="white">ri-checkbox-circle-line</VIcon>
              </VAvatar>
              <div>
                <div class="stat-label">تم الإرسال</div>
                <div class="stat-value">{{ statsLoading ? "…" : (stats.sent ?? 0) }}</div>
              </div>
            </div>
          </VCardItem>
        </VCard>
      ...
      <VCol cols="12" sm="6" lg="3">
        <VCard class="stat-card" elevation="0" rounded="lg" border>
          <VCardItem>
            <div class="d-flex align-center">
              <VAvatar color="warning" size="44" class="me-3">
                <VIcon color="white">ri-time-line</VIcon>
              </VAvatar>
              <div>
                <div class="stat-label">معلّقة</div>
                <div class="stat-value">{{ statsLoading ? "…" : (stats.pending ?? 0) }}</div>
              </div>
            </div>
          </VCardItem>
        </VCard>
      ...
      <VCol cols="12" sm="6" lg="3">
        <VCard class="stat-card" elevation="0" rounded="lg" border>
          <VCardItem>
            <div class="d-flex align-center">
              <VAvatar color="error" size="44" class="me-3">
                <VIcon color="white">ri-error-warning-line</VIcon>
              </VAvatar>
              <div>
                <div class="stat-label">فشل الإرسال</div>
                <div class="stat-value">{{ statsLoading ? "…" : (stats.failed ?? 0) }}</div>
              </div>
            </div>
          </VCardItem>
        </VCard>
    ...
    <VRow class="mb-2" dense v-if="stats && (Object.keys(stats.byType || {}).length || Object.keys(stats.byPriority || {}).length)">
      <VCol cols="12" md="6">
        <VCard elevation="0" rounded="lg" border>
          <VCardTitle class="d-flex align-center py-3 px-4">
            <VIcon color="primary" class="me-2">ri-pie-chart-line</VIcon>
            <span class="text-subtitle-1 font-weight-bold">حسب النوع</span>
          </VCardTitle>
          <VDivider />
          <VCardItem>
            <div v-for="(count, key) in stats.byType || {}" :key="key" class="breakdown-row">
              <VChip size="small" variant="tonal" color="primary">{{ key }}</VChip>
              <VSpacer />
              <span class="font-weight-bold">{{ count }}</span>
            </div>
            <div v-if="!Object.keys(stats.byType || {}).length" class="text-medium-emphasis text-center py-3">
              لا توجد بيانات
            </div>
          </VCardItem>
        </VCard>
      ...
      <VCol cols="12" md="6">
        <VCard elevation="0" rounded="lg" border>
          <VCardTitle class="d-flex align-center py-3 px-4">
            <VIcon color="warning" class="me-2">ri-flag-2-line</VIcon>
            <span class="text-subtitle-1 font-weight-bold">حسب الأولوية</span>
          </VCardTitle>
          <VDivider />
          <VCardItem>
            <div v-for="(count, key) in stats.byPriority || {}" :key="key" class="breakdown-row">
              <VChip size="small" variant="tonal" :color="priorityColor(key)">{{ priorityLabel(key) }}</VChip>
              <VSpacer />
              <span class="font-weight-bold">{{ count }}</span>
            </div>
            <div v-if="!Object.keys(stats.byPriority || {}).length" class="text-medium-emphasis text-center py-3">
              لا توجد بيانات
            </div>
          </VCardItem>
        </VCard>
    ...
    <VCard class="my-4" elevation="3" rounded="lg">
      <VCardTitle class="d-flex align-center py-4 px-6 flex-wrap gap-2">
        ...
        <h3 class="text-h5 font-weight-bold">إدارة الإشعارات</h3>
        <VSpacer />
        <VBtn color="warning" variant="tonal" prepend-icon="ri-play-circle-line" rounded="lg"
          :loading="processing" @click="processPending">
          معالجة المعلّقات الآن
        </VBtn>
        <VBtn color="primary" variant="tonal" prepend-icon="ri-refresh-line" rounded="lg"
          :loading="loading" @click="reloadAll">
          تحديث
        </VBtn>
      </VCardTitle>
      <VDivider />
      <VCardItem>
        <VRow dense>
          ...
          <VCol cols="12" md="3">
            <VSelect v-model="filters.priority" :items="priorityOptions" item-title="title" item-value="value"
              label="الأولوية" variant="outlined" density="comfortable" hide-details clearable @update:modelValue="fetchData" />
          </VCol>
          <VCol cols="12" md="3">
            <VTextField v-model="filters.type" placeholder="نوع الإشعار (اختياري)" prepend-inner-icon="ri-price-tag-3-line"
              variant="outlined" density="comfortable" hide-details clearable @keyup.enter="fetchData" @click:clear="fetchData" />
          </VCol>
          <VCol cols="12" md="3">
            <VSelect v-model="filters.recipientType" :items="recipientOptions" item-title="title" item-value="value"
              label="المستلم" variant="outlined" density="comfortable" hide-details clearable @update:modelValue="fetchData" />
          </VCol>
        </VRow>
      </VCardItem>
    </VCard>
    ...
    <VCard class="my-4" elevation="3" rounded="lg">
      <VCardTitle class="py-4 px-6 d-flex align-center">
        ...
        <h3 class="text-h6 font-weight-bold">قائمة الإشعارات</h3>
        <VSpacer />
        <VChip color="primary" variant="elevated">{{ totalItems }} إشعار</VChip>
      </VCardTitle>
      <VDivider />

      <VCardItem v-if="loading">
        <VSkeletonLoader v-for="n in 6" :key="n" type="list-item-avatar-three-line" class="mb-2" />
      </VCardItem>

      <VCardItem v-else-if="!items.length">
        <div class="text-center py-12 text-medium-emphasis">
          <VIcon size="56" color="grey-lighten-1" class="mb-3">ri-notification-off-line</VIcon>
          <h4 class="text-h6 mb-1">لا توجد إشعارات</h4>
          <p>عدّل الفلاتر أو انتظر إرسال إشعارات جديدة.</p>
        </div>
      </VCardItem>

      <VList v-else class="notif-list">
        <VListItem v-for="n in items" :key="n.id" class="notif-item">
          <template #prepend>
            <VAvatar :color="priorityColor(n.priority)" size="44" class="me-2">
              <VIcon color="white">{{ statusIcon(n.status) }}</VIcon>
            </VAvatar>
          </template>
          <VListItemTitle class="d-flex align-center flex-wrap gap-2">
            <strong class="text-body-1">{{ n.title }}</strong>
            <VChip size="x-small" variant="tonal" :color="statusColor(n.status)">{{ statusLabel(n.status) }}</VChip>
            <VChip size="x-small" variant="tonal" :color="priorityColor(n.priority)">{{ priorityLabel(n.priority) }}</VChip>
            <VChip v-if="n.type" size="x-small" variant="tonal" color="grey">{{ n.type }}</VChip>
            <VChip v-if="n.recipientType || n.recipient_type" size="x-small" variant="tonal" color="info">
              {{ recipientLabel(n.recipientType || n.recipient_type) }}
            </VChip>
          </VListItemTitle>
          <VListItemSubtitle class="mt-1">
            <div class="text-body-2 text-medium-emphasis notif-message">{{ n.message }}</div>
          ...
          <template #append>
            <VBtn icon="ri-delete-bin-line" size="small" color="error" variant="text" @click.stop="confirmDelete(n)" />
          </template>
        </VListItem>
      </VList>
    ...
    <BaseAlert v-if="alert.open" v-model="alert.open" :type="alert.type" :message="alert.message"
      :closable="true" close-text="موافق" @close="alert.open = false" />
  </div>
</template>
